...
function act(input) {
  ...
  if (action) {
    ...
    var reactionMilliseconds =
      ReactionMillisecondsLookup[action.spellId] ||
      defaultReactionMilliseconds(difficulty);
...
function defaultReactionMilliseconds(difficulty) {
  ...
  return (
    alpha * HardReactionMilliseconds + (1 - alpha) * EasyReactionMilliseconds
  );
}
...
function randomSpells(settings) {
  ...
  for (var btn in allOptions) {
    // One of the buttons, e.g. Q or R
...
function validAttack(state, hero, opponent, spell) {
  ...
  if (
    spell.action === "projectile" ||
    spell.action === "spray" ||
    spell.action === "retractor" ||
    spell.action === "focus" ||
    spell.action === "charge"
  ) {
    if (spell.id === "swap") {
      // Swap doesn't work as an attack
      return false;
    }
    if (opponentShielded && !spell.projectile.detonate) {
      // Detonate spells can penetrate shields, nothing else can
      return false;
    }
    if (spell.id === "whip") {
    ...
    var range =
      (spell.projectile.speed * spell.projectile.maxTicks) / TicksPerSecond +
      opponent.radius;
...
function updateNextSpellTime() {
  nextSpell =
    Date.now() +
    SpellCastIntervalMilliseconds +
    Math.floor(
      (Math.random() < 0.5 ? -1 : 1) *
        Math.random() *
        SpellCastJitterMilliseconds
    );
}
...
function focus(hero, opponent) {
  // When using a spell (e.g. Acolyte Beam, Spirit Missile) - ensure we are focusing the enemy, otherwise we will miss
  if (hero.casting) {
    if (
      hero.casting.spellId === "blast" ||
      hero.casting.spellId === "retractor" ||
      hero.casting.spellId === "rocket"
    ) {
      // Have to release or it won't fire
      return {
        spellId: hero.casting.spellId,
        release: true,
        target: opponent.pos,
      };
    } else if (
      hero.casting.spellId === "saber" ||
      hero.casting.spellId === "dualSaber"
    ) {
      ...
      return {
        ...
        target: vectorPlus(
          hero.pos,
          vectorFromAngle(hero.heading + Math.PI / 2)
        ),
      };
...
function dodge(state, hero, cooldowns) {
  for (var projectileId in state.projectiles) {
    ...
    var collisionPoint = vectorPlus(
      projectile.pos,
      vectorMultiply(projectile.velocity, timeToCollision)
    );
    ...
    var direction = vectorUnit(
      vectorNegate(vectorDiff(collisionPoint, hero.pos))
    );
